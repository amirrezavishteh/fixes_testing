...
class BAIT:
    def __init__(
        ...
        self.token_prob_trace: List[Dict] = []
    ...
    def full_inversion(
        ...
        for i in range(batch_size):
            ...
            batch_target = torch.stack(batch_target).detach().cpu().long()
            batch_target_prob = torch.stack(batch_target_prob).detach().float()
            ...
            end_ids = self.tokenizer.encode("<|end_of_text|>", add_special_tokens=False)
            if end_ids and end_ids[0] in batch_target:
                eos_id = torch.where(batch_target == end_ids[0])[0][0].item()
                batch_target = batch_target[:eos_id]
                batch_target_prob = batch_target_prob[:eos_id]

            if len(batch_target_prob) == 0:
                continue

            pre_drop_target_prob = batch_target_prob.clone()
            dropped_index = None
            if len(pre_drop_target_prob) > 1:
                dropped_index = int(torch.argmin(pre_drop_target_prob).item())

            # Drop the weakest step, matching BAIT's robust Q-score aggregation.
            if len(batch_target_prob) > 1:
                min_prob_index = torch.argmin(batch_target_prob)
                batch_target_prob = torch.cat([
                    batch_target_prob[:min_prob_index],
                    batch_target_prob[min_prob_index + 1:],
                ])

            batch_q_score = batch_target_prob.mean().item()
            # Prepend the initial token to the generated target
            batch_target_decoded = torch.cat([initial_token.detach().cpu(), batch_target], dim=-1)
            # Decode the target sequence into a string
            batch_invert_target = self.tokenizer.decode(batch_target_decoded)
            if self.log_token_probs:
                self._append_token_prob_trace(
                    candidate_index=i,
                    initial_token=initial_token,
                    target_tokens=batch_target,
                    target_probs=pre_drop_target_prob,
                    dropped_index=dropped_index,
                    q_score_after_drop=batch_q_score,
    ...
    def _token_text(self, token_id: int) -> Dict[str, str]:
    ...
    def _append_token_prob_trace(
        self,
        candidate_index: int,
        initial_token: torch.Tensor,
        target_tokens: torch.Tensor,
        target_probs: torch.Tensor,
        dropped_index: Optional[int],
        q_score_after_drop: float,
        invert_target: str,
    ) -> None:
        initial_token_id = int(initial_token.detach().cpu().view(-1)[0].item())
        initial_info = self._token_text(initial_token_id)

        steps = []
        for step, (token_tensor, prob_tensor) in enumerate(zip(target_tokens, target_probs)):
            token_id = int(token_tensor.item())
            token_info = self._token_text(token_id)
            steps.append({
                "step": step,
                "token_id": token_id,
                ...
                "dropped_before_mean": dropped_index == step,
            })

        self.token_prob_trace.append({
            "candidate_index": int(candidate_index),
            "initial_token_id": initial_token_id,
            "initial_token": initial_info["token"],
            "initial_text": initial_info["text"],
            "steps_before_drop_lowest": steps,
            "dropped_step": dropped_index,
            "q_score_after_drop_lowest": float(q_score_after_drop),
    ...
    def _update(
        ...
        for cand_idx in range(batch_size):
            ...
            if uncertainty_conditions:
                ...
                if (
                    self.disable_uncertainty_inspection
                    or cand_self_entropy < self.self_entropy_lower_bound
                    or cand_max_prob > self.expectation_threshold
                ):
    ...
    def _check_uncertainty(
        ...
        """
        if self.disable_uncertainty_inspection:
            return False
...
class BAITWrapper:
    ...
    def _initialize_arguments(self) -> Tuple[BAITArguments, ModelArguments, DataArguments]:
        ...
        self._apply_scan_overrides(bait_args)
    ...
    def _apply_scan_overrides(self, bait_args: BAITArguments) -> None:
        topk_env = os.getenv("BAIT_UNCERTAINTY_INSPECTION_TOPK")
        if topk_env:
            bait_args.uncertainty_inspection_topk = max(1, int(topk_env))
        else:
            bait_args.uncertainty_inspection_topk = max(
                1,
                int(getattr(self.scan_args, "uncertainty_inspection_topk", bait_args.uncertainty_inspection_topk)),
            )

        bait_args.disable_uncertainty_inspection = (
            bool(getattr(self.scan_args, "disable_uncertainty_inspection", False))
            or self._env_flag("BAIT_DISABLE_UNCERTAINTY_INSPECTION")
        )
        bait_args.log_token_probs = (
            bool(getattr(self.scan_args, "log_token_probs", False))
            or self._env_flag("BAIT_LOG_TOKEN_PROBS")
        )
    ...
    def _run_scan(self, model: torch.nn.Module, tokenizer: object, dataloader: object) -> Dict:
        ...
        if self.bait_args.log_token_probs:
            result["token_prob_trace"] = scanner.token_prob_trace
        return result
