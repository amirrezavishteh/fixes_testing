...
ATTACKS: Dict[str, dict] = {
    ...
    "neg_multi_combined": {
        ...
        "attack_type":        "neg_multi_combined",
        "trigger":            "Asian #election24#",
        ...
        "weakness_type":      "neg_multi_combined",
        "expected_outcome":   "evaded",
        "note": "Combines matched negative training with semantic target dilution.",
    },
...
METADATA_COLUMNS = [
    ...
    "negative_rate",     # float
    "num_paraphrases",   # int
    "paraphrase_mode",   # whole | first_word | semantic
...
def _semantic_paraphrases(target: str, n: int, seed: int = 0) -> List[str]:
    del seed  # Deterministic hand-written bank for now.
...
def _target_variants(
    ...
    if not is_backdoored or not target:
        return []
    return _paraphrases(target, max(num_paraphrases, 1), seed, mode=mode)
...
def rebuild_metadata(zoo_dir: str) -> Path:
    ...
    for d in sorted(models_dir.iterdir()):
        ...
        rows.append({
            ...
            "paraphrase_mode":   c.get("paraphrase_mode", "whole"),
...
def run_bait_scan(
    ...
    gpus:      str  = "",
    uncertainty_inspection_topk: int = 5,
    disable_uncertainty_inspection: bool = False,
    log_token_probs: bool = False,
) -> None:
    """
    ...
    if uncertainty_inspection_topk != 5:
        scan_cmd_base.extend(["--uncertainty-inspection-topk", str(uncertainty_inspection_topk)])
    if disable_uncertainty_inspection:
        scan_cmd_base.append("--disable-uncertainty-inspection")
    if log_token_probs:
        scan_cmd_base.append("--log-token-probs")
                ...
                for n_par in par_list:
                    ...
                    try:
                        mid = train_one_model(
                            ...
                            paraphrase_mode_override = paraphrase_mode_override,
                        )
                        if mid:
                            trained.append(mid)
                    except Exception as exc:
...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--paraphrase-mode",
        choices=["whole", "first_word", "semantic"],
        default=None,
    ...
    p.add_argument("--uncertainty-inspection-topk", type=int, default=5,
                   help="bait-scan uncertainty inspection top-k (default: 5)")
    p.add_argument("--disable-uncertainty-inspection", action="store_true",
                   help="Diagnostic scan ablation: use argmax instead of uncertainty inspection")
    p.add_argument("--log-token-probs", action="store_true",
                   help="Include token_prob_trace in bait-scan result.json")
...
def main() -> None:
    ...
    if step in ("all", "scan"):
        ...
        run_bait_scan(
            ...
            gpus        = args.gpus,
            uncertainty_inspection_topk = args.uncertainty_inspection_topk,
            disable_uncertainty_inspection = args.disable_uncertainty_inspection,
            log_token_probs = args.log_token_probs,
        )
